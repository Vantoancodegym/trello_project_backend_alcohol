...
public interface BoardRepo extends JpaRepository<Board,Long> {
    @Query(value = "select board.id, board.name, board.app_user_id as appUserId from board join tag_user_board tub on board.id = tub.board_id where tub.app_user_id =?", nativeQuery = true)
